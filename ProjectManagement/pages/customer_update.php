...
$f_association_mode = gpc_get_int( 'association_mode', 0 );

$t_customer_table = plugin_table( 'customer' );

if ( empty( $f_customer_id ) ) {
	# Insert new customer
	$t_query = "INSERT INTO $t_customer_table (name, share, can_approve, association_mode)
				VALUES ('$f_name', $f_share, $f_can_approve, $f_association_mode)";
	db_query_bound( $t_query );
} else {
	# Update existing customer
	$t_query = "UPDATE $t_customer_table
				   SET name = '$f_name', share = $f_share, can_approve = $f_can_approve, association_mode = $f_association_mode

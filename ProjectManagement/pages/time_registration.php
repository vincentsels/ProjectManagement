...
foreach ( $t_worktypes as $t_worktype_code => $t_worktype_label ) {
	...
	$f_data[PLUGIN_PM_EST][$t_worktype_code] = time_to_minutes( gpc_get_string( 'change_' . PLUGIN_PM_EST . '_' . $t_worktype_code ) );
	$f_data[PLUGIN_PM_DONE][$t_worktype_code] = time_to_minutes( gpc_get_string( 'add_' . PLUGIN_PM_DONE . '_' . $t_worktype_code ) );
	$f_data[PLUGIN_PM_TODO][$t_worktype_code] = time_to_minutes( gpc_get_string( 'change_' . PLUGIN_PM_TODO . '_' . $t_worktype_code ) );
	$f_data['clear_todo'][$t_worktype_code] = gpc_get_bool( 'clear_' . PLUGIN_PM_TODO . '_' . $t_worktype_code );
}

# Handle estimations: insert or update
foreach ( $f_data[PLUGIN_PM_EST] as $t_worktype => $t_minutes ) {
	if ( !empty( $t_minutes ) ) {
		set_work( $f_bug_id, $t_worktype, PLUGIN_PM_EST, $t_minutes, Action::INSERT_OR_UPDATE );
	}
}
...
foreach ( $f_data[PLUGIN_PM_DONE] as $t_worktype => $t_minutes ) {
	if ( !empty( $t_minutes ) ) {
		set_work( $f_bug_id, $t_worktype, PLUGIN_PM_DONE, $t_minutes, Action::INSERT );
	}
}
...
foreach ( $f_data[PLUGIN_PM_TODO] as $t_worktype => $t_minutes ) {
	if ( !empty( $t_minutes ) ) {
		set_work( $f_bug_id, $t_worktype, PLUGIN_PM_TODO, $t_minutes, Action::INSERT_OR_UPDATE );
	}
}
...
foreach ( $f_data['clear_todo'] as $t_worktype => $t_minutes ) {
	if ( !empty( $t_minutes ) ) {
		set_work( $f_bug_id, $t_worktype, PLUGIN_PM_TODO, $t_minutes, Action::DELETE );
	}
}
